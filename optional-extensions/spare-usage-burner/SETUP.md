...
9. If Templater folder templates are configured (core setup), add the line `"projects-claude": "templates/project.md",` to `FOLDER_TEMPLATES` in `templates/startup-folder-templates.md`, next to the `projects` one (takes effect after an Obsidian reload).
