...
3. **Templater**: the same [[project]] folder template applies here as in `projects/` (empty click-created files only; mapping in [[startup-folder-templates]]) — mostly irrelevant in practice since Claude writes complete files itself: `status`/`parent` frontmatter, and `# Goal clarification` / `# Subtasks` / `# Notes` sections on goal notes.
