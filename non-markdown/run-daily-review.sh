...
claude_args=(--permission-mode acceptEdits --settings non-markdown/scheduled-settings.json --strict-mcp-config)
...
if [ -x non-markdown/run-vault-cleanup.sh ]; then
  non-markdown/run-vault-cleanup.sh || echo "WARN: vault cleanup failed — continuing with the review"
fi

# ---- daily -------------------------------------------------------------------
# Keep .gitignore's managed external-projects block current before staging.
if [ -x non-markdown/update-external-ignores.sh ]; then
  non-markdown/update-external-ignores.sh || echo "WARN: update-external-ignores failed"
fi
