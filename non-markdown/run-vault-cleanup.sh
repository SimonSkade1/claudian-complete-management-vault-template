...
# file, not a skill, so it costs no context until read): restores the projects/
# folder tree from the goal tree, relocates handoffs, patches path references
# the moves break, and refreshes the INDEX files and projects/overview.md.
#
...
LOG_NOTE="tasks-and-notes/vault-cleanup log.md"
...
claude_args=(--permission-mode acceptEdits --settings non-markdown/cleanup-settings.json --strict-mcp-config)

logged_today() {  # the skill stamps last_run — that is the success signal
