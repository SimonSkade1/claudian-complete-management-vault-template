...
Base files are YAML defining database-like views over vault notes. This vault's main bases: `me.base` (tasks, views incl. Do / Wait & Subscribed / Later / Notes) and `claude.base` (Claude task queue, Do view). `projects/` has no base — its dashboard is `projects/overview.md`.
...
```bash
obsidian base:query path="me.base" view="Do" format=json
```

Paths relative to vault root. Formats: `json|csv|tsv|md|paths`. The CLI needs the Obsidian app running.

**Fallback** (app closed / CLI unavailable): `python3 .claude/scripts/query-base.py <path> [view]`
