...
Generates the review notes in `auto-review/`. Normally run headless by the scheduled pipeline (`non-markdown/run-daily-review.sh`, via the `daily-review.timer` systemd user unit on the user's laptop); can also be invoked manually.
